...
meta <- get_bibloadr_meta(varfile = varfile, varlist = varlist, srclist = srclist, subcohort = subcohort, type = "varsection")
sourcemeta <- get_bibloadr_meta(varfile = varfile, varlist = varlist, srclist = srclist, subcohort = subcohort, type = "sourcelong")
codebook <- get_bibloadr_meta(varfile = varfile, varlist = varlist, srclist = srclist, subcohort = subcohort, type = "code")
stats <- get_bibloadr_stats(varfile = varfile, varlist = varlist, subcohort = subcohort, srclist = srclist)
meta <- merge(meta, stats, by = "VariableName")
sourcestats <- select(meta, SourceName, NRows, MultiObs, NUnits, Hidden) %>% 
               group_by(SourceName) %>% mutate(NUnits = max(NUnits), NVars = n(), NHidden = sum(Hidden)) %>%
               select(-Hidden) %>% unique()
...
vardescription <- function(var, src) {
  ...
  if(var$Hidden == 1) value_line <- paste0("_[Hidden]_ ", value_line)
...
Database ID for source: `{{src$SourceName}}`

This source is measured at the **{{tolower(src$MeasurementLevel)}}** level. It contains data from {{src$NUnits}} {{plural_level}} with {{ifelse(src$MultiObs==1,'more than','')}} one observation per {{tolower(src$MeasurementLevel)}}. There are {{src$NVars}} variables with a total of {{src$NRows}} observations. {{ifelse(src$NHidden>0,paste0(src$NHidden,' variable', ifelse(src$NHidden>1,'s are',' is'), ' sensitive or potentially disclosive and will be hidden from standard data packages. ', ifelse(src$NHidden>1,'These are','This is'), ' marked as _Hidden_, below. '),'')}}
...
This document is a data dictionary for `r data_package_name`. It describes `r nrow(meta)` variables from `r nrow(sourcemeta)` source`r ifelse(nrow(sourcemeta)>1,"s","")`. It was built from Born in Bradford database version `r database_version`.
